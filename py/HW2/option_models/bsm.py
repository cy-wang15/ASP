...
class BsmModel:
    ...
    def delta(self, strike, spot, texp, cp_sign=1):
        d1 = (np.log(spot/strike)+intr*texp+0.5*vol**2*T)/(vol*np.sqrt(texp))
        d2 = (np.log(spot/strike)+intr*texp+0.5*vol**2*T)/(vol*np.sqrt(texp))
        return cp_sign*ss.norm.cdf(cp_sign*d1)
